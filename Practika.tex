...
Запишем в каждом из кругов ноль или единицу. После этого в каждом лепестке запишем остаток при делении на два суммы чисел во всех кругах, в которых содержится этот лепесток.
\end{frame}
...
\begin{frame}
Описанная схема называется кодом Хэмминга и обладает интересным свойством. Если ваш враг в тайне от вас изменит любое из семи записанных по этой схеме чисел, вы сможете однозначно определить, какое число он изменил.
\end{frame}
...
В единственной строке через пробел записаны семь чисел. Каждое из чисел равно нулю или единице.
        \end{itemize}
...
\begin{frame}{Решение задачи}
        \textbf{Так как на вход мы получаем семь чисел, то разложив их как три суммы с соответствующими тремя значениями, и одновременно сравних их верность, мы получим либо код Хэмминга, либо ошибку.}
\end{frame}

\begin{frame}
        \textbf{И так как в условии задачи сказано что необходимо изменить только одно число, то самый простой способ это перебор всех чисел подряд, то есть мы просто меняем первую цифру на противоположную и снова проверяем изначальное условие, если ответ утвердителен, код найден, в противном случае возвращаем значение первой цифры и меняем уже вторую, и так мы делаем до самой последней цифры, когда она поменяется в значении, мы найдём код Хэмминга.}
\end{frame}
